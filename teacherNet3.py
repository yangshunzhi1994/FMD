...
class Teacher3(nn.Module):
    def __init__(self, ResNet_factor=8, num_classes=7):
        ...
        #self.res1 = self.make_layer(resblock, 4, nChannels, nChannels)
    ...
    def forward(self, x):
        ...
        out = F.relu(rb3, inplace=True)
        mimic = F.avg_pool2d(out, kernel_size=11).view(out.size(0), -1)
        out = self.fc(mimic)
        return rb1, rb2, rb3, mimic, out
